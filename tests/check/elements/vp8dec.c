...
GST_START_TEST (test_decode_invalid_resolution)
{
  ...
  fail_unless_equals_int (GST_FLOW_OK,
      gst_harness_push (h, gst_buffer_new_wrapped_full (
              GST_MEMORY_FLAG_READONLY, invalid_width, sizeof (invalid_width),
              0, sizeof (invalid_width), NULL, NULL)));
  fail_unless(gst_harness_try_pull (h) == NULL);

  fail_unless_equals_int (GST_FLOW_OK,
      gst_harness_push (h, gst_buffer_new_wrapped_full (
              GST_MEMORY_FLAG_READONLY, invalid_height, sizeof (invalid_height),
              0, sizeof (invalid_height), NULL, NULL)));
  fail_unless(gst_harness_try_pull (h) == NULL);
