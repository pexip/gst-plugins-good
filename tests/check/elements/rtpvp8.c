...
GST_START_TEST (test_depay_incomplete_frame_before_picid_gap)
{
  ...
  /* FIXME: hide-picture-id-gap doesn't really hide a gap in picid, it only
   * notifies in the resulting gap event that there was no actual packet loss */
...
GST_START_TEST (test_no_marker_bits)
{
  GstClockTime pts = 0;
  gint seqnum = 100;
  gint i;
...
GST_START_TEST (test_depay_incomplete_frames_no_gaps)
{
  gint i;
  GstClockTime pts = 0;
  GstHarness *h = gst_harness_new ("rtpvp8depay");
...
rtpvp8_suite (void)
{
  ...
  tcase_add_loop_test (tc_chain, test_depay_incomplete_frame_before_picid_gap, 0, 2);
  tcase_add_test (tc_chain, test_no_marker_bits);
  tcase_add_test (tc_chain, test_depay_incomplete_frames_no_gaps);
