    ...
    guint16 *dst_lost_seq, GstClockTime *dst_now)
{
  ...
  for (frame = 0, seq = 0, now = 0; frame < 2; frame++, seq += 2, now += frame_dur) {
    gst_harness_set_time (h, now);
    gst_harness_push (h, generate_test_buffer_full (now, seq, AS_TEST_BUF_RTP_TIME (now)));
    gst_harness_push (h, generate_test_buffer_full (now, seq + 1, AS_TEST_BUF_RTP_TIME (now)));
  ...
  gst_harness_set_time (h, now);
  buffer = generate_test_buffer_full (now, seq, AS_TEST_BUF_RTP_TIME (lost_packet_time));
  GST_BUFFER_FLAG_SET (buffer, GST_RTP_BUFFER_FLAG_RETRANSMISSION);
...
GST_START_TEST (test_rtx_large_packet_spacing_and_small_rtt)
{
  ...
  fail_unless_equals_int64 (now + (latency_ms - rtx_rtt_ms) * GST_MSECOND,
      gst_clock_get_time (GST_ELEMENT_CLOCK (h->element)));
  verify_rtx_event (h, lost_seq, now, (latency_ms - rtx_rtt_ms), frame_dur_ms * GST_MSECOND);
...
GST_START_TEST (test_big_gap_seqnum)
{
  ...
  for (i = 0; i < num_consecutive; i++) {
    fail_unless_equals_int (GST_FLOW_OK, gst_harness_push (h,
        generate_test_buffer_full (
            dts_base + i * TEST_BUF_DURATION,
            seqnum_base + i,
            rtpts_base + i * TEST_RTP_TS_DURATION)));
  }
  ...
  for (i = 0; i < num_consecutive; i++) {
    ...
    GstClockTime expected_ts = dts_base + i * TEST_BUF_DURATION;
...
rtpjitterbuffer_suite (void)
{
  ...
  tcase_add_test (tc_chain, test_rtx_large_packet_spacing_does_not_reset_jitterbuffer);
