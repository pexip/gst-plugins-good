...
GST_START_TEST (test_considered_lost_packet_in_large_gap_arrives)
{
  ...
  GstEvent *event;
  gint already_lost_seqnum = __i__;
   ...
   * processed */
  fail_unless_equals_int (GST_FLOW_OK,
      gst_harness_push (h, generate_test_buffer (already_lost_seqnum)));

  /* time out for lost packets 1 and 2 (one event, double duration) */
  fail_unless (gst_harness_crank_single_clock_wait (h));
  event = gst_harness_pull_event (h);
  verify_lost_event (event, 1, 1 * PCMU_BUF_DURATION, 2 * PCMU_BUF_DURATION);

  /* time out for lost packets 3 */
  fail_unless (gst_harness_crank_single_clock_wait (h));
  event = gst_harness_pull_event (h);
  verify_lost_event (event, 3, 3 * PCMU_BUF_DURATION, 1 * PCMU_BUF_DURATION);
...
rtpjitterbuffer_suite (void)
{
  ...
  tcase_add_loop_test (tc_chain,
      test_considered_lost_packet_in_large_gap_arrives, 1, 3);
