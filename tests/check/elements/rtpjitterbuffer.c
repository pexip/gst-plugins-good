...
static void
verify_gap_event (GstHarness * h, GstClockTime exp_ts,
    GstClockTime exp_dur)
{
  GstEvent *event;
  const GstStructure *s;
...
{
  return construct_deterministic_initial_state_full (h, latency_ms, generate_caps ());
}
...
generate_dtx_caps (void)
{
  return gst_caps_new_simple ("application/x-rtp",
      ...
      "dtx-duration", G_TYPE_UINT64, TEST_BUF_DURATION,
      NULL);
}
...
{
  return construct_deterministic_initial_state_full (h, latency_ms, generate_dtx_caps ());
}
