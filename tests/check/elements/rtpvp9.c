...
GST_START_TEST (test_depay_stop_gap_events_pictureid_wraps)
{
  ...
  if (mbit) {
    intra[13] = 0xff; // Mbit=1 + 0x7fff pictureid
    intra[14] = 0xff;
    inter[13] = 0x80; // Mbit=1 + 0x000 pictureid
    inter[14] = 0x00;
  } else {
    intra[13] = 0x7f; // Mbit=0 + 0x7f pictureid
    inter[13] = 0x00; // Mbit=0 + 0x00 pictureid
  }
...
rtpvp9_suite (void)
{
  ...
  tcase_add_loop_test (tc_chain, test_depay_stop_gap_events_pictureid_wraps, 0, G_N_ELEMENTS (test_data_no_gap_in_picid));
