...
static GstBuffer *
caps_stress_push_buffer_cb (G_GNUC_UNUSED GstHarness * h, gpointer data)
{
  gint *seqnum = data;
...
GST_START_TEST (test_rtxsender_caps_stress)
{
  ...
  /* disable test for valgrind, as it takes way too long */
#ifdef HAVE_VALGRIND
  ...
  h = gst_harness_new ("rtprtxsend");
  probe_id = gst_pad_add_probe (h->sinkpad, GST_PAD_PROBE_TYPE_BUFFER,
      probe_check_caps_vs_rtp_data, NULL, NULL);
  pt_map = gst_structure_new ("application/x-rtp-pt-map", "96", G_TYPE_UINT, 99,
      NULL);
  g_object_set (h->element, "payload-type-map", pt_map, NULL);
  gst_structure_free (pt_map);
