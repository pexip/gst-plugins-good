...
static void
on_app_rtcp_cb (GObject *session, guint subtype, guint ssrc, const gchar *name,
    GstBuffer *data, RTCPAppResult *result)
{
...
GST_START_TEST (test_creating_srrr)
{
  ...
  for (i = 0; i < 5; i++) {
    ...
    res = gst_pad_push (data.src,
        generate_test_buffer (i * 20 * GST_MSECOND, FALSE, i, i*20, 0x01BADBAD));
    fail_unless (res == GST_FLOW_OK || res == GST_FLOW_FLUSHING);
...
suspicious_bye_cb (GObject *object, GParamSpec *spec, gpointer data)
{
  ...
  *cb_called = TRUE;
  ...
  stats_arr = g_value_get_boxed (gst_structure_get_value (stats, "source-stats"));
  g_assert (stats_arr != NULL);
  fail_unless_equals_int (stats_arr->n_values, 1);
  internal_stats = g_value_get_boxed (
    g_value_array_get_nth (stats_arr, 0));
  g_assert (internal_stats != NULL);

  gst_structure_get (internal_stats,
        "ssrc", G_TYPE_UINT, &ssrc,
        "internal", G_TYPE_BOOLEAN, &internal,
        "received-bye", G_TYPE_BOOLEAN, &sent_bye,
        NULL);
...
GST_START_TEST (test_ignore_suspicious_bye)
{
  ...
  GstElement * internal_session = NULL;
  GstTestClock * testclock = GST_TEST_CLOCK (gst_test_clock_new ());
  ...
  g_object_get (h_rtcp->element, "internal-session", &internal_session, NULL);
  g_object_set (internal_session, "rtcp-min-interval", 1, NULL);
