...
static void
new_payload_type (GstElement *element, guint pt, GstPad * pad, GstHarness **h)
{
...
test_rtpptdemux_srccaps_from_sinkcaps_base (
    const gchar *srccaps, const gchar *sinkcaps)
{
  GstCaps *caps;
  gchar *caps_str;
  ...
  gst_buffer_unref (
    gst_harness_push_and_pull (h, gst_rtp_buffer_new_allocate(0,0,0)));
...
static GstCaps*
request_pt_map (GstElement *demux, guint pt, const gchar *caps)
{
   (void)pt;
   (void)demux;
...
test_rtpptdemux_srccaps_from_signal_base(
    const gchar *srccaps, const gchar *sigcaps, const gchar *sinkcaps)
{
  GstCaps *caps;
  gchar *caps_str;
  GstHarness *h = gst_harness_new_with_padnames ("rtpptdemux", "sink", NULL);
