...
#if 1                           /* undefine for old flvmux, since it does not wait on the clock */
    if (i != 0) {
      GstClockID pending;
      gst_test_clock_wait_for_next_pending_id (tclock, &pending);
      /* check if we should release the wait */
      if (gst_clock_id_get_time (pending) <= now) {
        GstClockID res = gst_test_clock_process_next_clock_id (tclock);
        fail_unless (res == pending);
