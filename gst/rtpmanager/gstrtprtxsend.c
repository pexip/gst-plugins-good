...
gst_rtp_rtx_send_token_bucket (GstRtpRtxSend * rtx, GstBuffer * buf)
{
  ...
  rtx->bucket_size = MIN (G_MAXINT, rtx->bucket_size + tokens);
  GST_LOG_OBJECT (rtx, "Adding %d tokens to bucket (contains %lu tokens)",
      tokens, rtx->bucket_size);

  if (rtx->max_kbps != -1 && rtx->bucket_size > rtx->max_kbps * 1000)
    rtx->bucket_size = rtx->max_kbps * 1000;
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MAX_KBPS:
      GST_OBJECT_LOCK (rtx);
      rtx->max_kbps = g_value_get_int (value);
      if (rtx->max_kbps != -1)
        rtx->bucket_size = rtx->max_kbps * 1000;
      GST_OBJECT_UNLOCK (rtx);
      break;
