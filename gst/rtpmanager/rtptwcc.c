...
void
rtp_twcc_packet_init (RTPTWCCPacket * packet, RTPPacketInfo * pinfo)
{
...
{
  gint32 seqa = ((RTPTWCCPacket *) a)->seqnum;
  gint32 seqb = ((RTPTWCCPacket *) b)->seqnum;
  gint res = seqa - seqb;
  if (res < -65000)
    res = 1;
  if (res > 65000)
...
static void
run_lenght_helper_update (RunLengthHelper * rlh, RTPTWCCPacket * pkt)
{
...
void
rtp_twcc_add_fci (GstRTCPPacket * packet, GArray * twcc_packets,
    guint8 fb_pkt_count)
{
  RTPTWCCPacket *first, *last, *prev;
  ...
  g_array_sort (twcc_packets, _twcc_seqnum_sort);

  /* get first and last packet */
  first = &g_array_index (twcc_packets, RTPTWCCPacket, 0);
  ...
  rtp_twcc_write_chunks (packet_chunks, twcc_packets,
  ...
  rtp_twcc_write_recv_deltas (fci_data_ptr, twcc_packets);
...
rtp_twcc_parse_fci (guint8 * fci_data, guint fci_length)
{
  ...
  for (i = 0; i < twcc_packets->len && fci_parsed < fci_length; i++) {
    ...
    GST_DEBUG ("parsed: pkt: #%u, ts: %" GST_TIME_FORMAT " status: %u",
