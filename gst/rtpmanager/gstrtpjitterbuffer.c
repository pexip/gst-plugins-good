...
gst_rtp_jitter_buffer_init (GstRtpJitterBuffer * jitterbuffer)
{
  ...
  priv->jbtimers = jb_timers_new (priv->rtx_delay_reorder);
...
gst_rtp_jitter_buffer_set_clock (GstElement * element, GstClock * clock)
{
  ...
  if (clock) {
    rtp_jitter_buffer_set_pipeline_clock (jitterbuffer->priv->jbuf,
        gst_object_ref (clock));

    jb_timers_set_clock_and_base_time (jitterbuffer->priv->jbtimers,
        gst_object_ref (clock), element->base_time);
    ...
    GstCaps * caps)
{
  ...
  if (gst_structure_get_clock_time (caps_struct, "npt-stop", &tval))
    ...
    priv->npt_stop = GST_CLOCK_TIME_NONE;
...
queue_event (GstRtpJitterBuffer * jitterbuffer, GstEvent * event)
{
  ...
  item = alloc_item (event, ITEM_TYPE_EVENT, GST_CLOCK_TIME_NONE, GST_CLOCK_TIME_NONE, -1, 0, -1);
...
static TimerData *
find_timer (JBTimers * jbtimers, guint16 seqnum)
{
...
static void
unschedule_timer_thread_sync (JBTimers * jbtimers)
{
  if (jbtimers->clock_id) {
    GST_DEBUG ("waking up timer thread");
...
static void
timer_set_timeout (GstRtpJitterBuffer * jitterbuffer, TimerData * timer, GstClockTime timeout, GstClockTime delay)
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  ...
  if (timer->type == TIMER_TYPE_LOST)
      timer->lost_packet_pts = timer->timeout;
...
static void
unschedule_timer_thread_sync_if_timer_expired (JBTimers * jbtimers, TimerData * timer)
{
  if (jbtimers->clock_id) {
    ...
    GstClockTime duration)
{
  ...
  timer->seqnum = seqnum;
  timer->num = num;
  timer_set_timeout (jitterbuffer, timer, timeout, delay);
    ...
    guint16 seqnum, GstClockTime timeout, GstClockTime delay, gboolean reset)
{
  ...
  oldseq = timer->seqnum;
  old_timeout = timer->timeout;

  timer_set_timeout (jitterbuffer, timer, timeout, delay);

  seqchange = oldseq != seqnum;
  timechange = old_timeout != timer->timeout;
  ...
  GST_DEBUG_OBJECT (jitterbuffer,
      ...
      GST_TIME_ARGS (old_timeout), GST_TIME_ARGS (timer->timeout));
...
static gboolean
already_lost (JBTimers * jbtimers, guint16 seqnum)
{
  gint i, len;
    ...
    for (i = 0; i < len; i++) {
      ...
      if (gap > priv->rtx_delay_reorder) {
        ...
        if (test->num_rtx_retry == 0 && test->type == TIMER_TYPE_EXPECTED)
          reschedule_timer (jitterbuffer, test, test->seqnum, GST_CLOCK_TIME_NONE, 0, FALSE);
      }
    }
  }
    ...
    GstClockTime pts)
{
  ...
  if (priv->ips_rtptime != rtptime) {
    ...
    if (priv->ips_pts != GST_CLOCK_TIME_NONE && pts != GST_CLOCK_TIME_NONE && pts > priv->ips_pts) {
    ...
    guint16 seqnum, GstClockTime pts, gint gap)
{
  ...
  if (priv->do_retransmission) {
    TimerData *timer = find_timer (priv->jbtimers, expected);
    ...
    guint32 rtptime)
{
  ...
  if (priv->last_dts == GST_CLOCK_TIME_NONE && priv->last_rtptime == -1)
    goto done;
    ...
    GstPad * pad, GstObject * parent, guint16 seqnum)
{
  ...
  priv->last_in_pts = GST_CLOCK_TIME_NONE;
  priv->next_in_seqnum = -1;
  ...
  for (l = events; l; l = l->next) {
    ...
    item = alloc_item (l->data, ITEM_TYPE_EVENT, GST_CLOCK_TIME_NONE, GST_CLOCK_TIME_NONE, -1, 0, -1);
    rtp_jitter_buffer_insert (priv->jbuf, item, &head, NULL);
  }
  g_list_free (events);
...
gst_rtp_jitter_buffer_fast_start (GstRtpJitterBuffer * jitterbuffer)
{
  ...
  timer = find_timer (priv->jbtimers, item->seqnum);
  if (!timer || timer->type != TIMER_TYPE_DEADLINE)
    return FALSE;
    ...
    GstBuffer * buffer)
{
  ...
  if (G_UNLIKELY (expected == -1)) {
    ...
    if (G_UNLIKELY (gap > 0 && priv->jbtimers->timers->len >= max_dropout)) {
      ...
      GST_WARNING_OBJECT (jitterbuffer, "%d pending timers > %d - resetting",
          priv->jbtimers->timers->len, max_dropout);
      gst_buffer_unref (buffer);
  ...
  timer = find_timer (priv->jbtimers, seqnum);
  if (GST_BUFFER_IS_RETRANSMISSION (buffer)) {
    if (!timer)
      timer = timer_queue_find (priv->jbtimers->rtx_stats_timers, seqnum);
    if (timer)
  ...
  if (already_lost (priv->jbtimers, seqnum))
    goto already_lost;
    ...
    RTPJitterBufferItem * item)
{
  ...
  if (priv->npt_stop == GST_CLOCK_TIME_NONE || priv->ext_timestamp == GST_CLOCK_TIME_NONE
      || priv->clock_base == GST_CLOCK_TIME_NONE || priv->clock_rate <= 0)
    return;
  ...
  if (estimated != GST_CLOCK_TIME_NONE && priv->estimated_eos != estimated) {
...
get_rtx_retry_timeout (GstRtpJitterBufferPrivate * priv)
{
  ...
  if (priv->rtx_retry_timeout == GST_CLOCK_TIME_NONE) {
  ...
  if (priv->rtx_min_retry_timeout == GST_CLOCK_TIME_NONE) {
    ...
    GstClockTime rtx_retry_timeout)
{
  ...
  if (priv->rtx_retry_period == GST_CLOCK_TIME_NONE) {
    ...
    GstClockTime now)
{
  ...
  if (gst_rtp_buffer_compare_seqnum (priv->next_in_seqnum, next_in_seqnum) > 0) {
    ...
    priv->last_in_pts = apply_offset (jitterbuffer, timer->lost_packet_pts);
  }
  ...
  if (priv->do_lost) {
    ...
    timestamp = apply_offset (jitterbuffer, timer->lost_packet_pts);
  ...
  item = alloc_item (event, ITEM_TYPE_LOST, GST_CLOCK_TIME_NONE, GST_CLOCK_TIME_NONE, seqnum, lost_packets, -1);
  ...
  while (jbtimers->timer_running) {
    ...
    for (i = 0; i < len;) {
      ...
      if (test->type == TIMER_TYPE_LOST &&
        ...
        if (timer == NULL) {
          save_best = TRUE;
        } else if (timer_timeout == GST_CLOCK_TIME_NONE) {
          ...
          if (test_timeout == GST_CLOCK_TIME_NONE
              && (gst_rtp_buffer_compare_seqnum (test->seqnum,
                      timer->seqnum) > 0))
            save_best = TRUE;
        } else if (test_timeout == GST_CLOCK_TIME_NONE) {
    ...
    if (timer && !priv->blocked) {
      ...
      if (timer_timeout == GST_CLOCK_TIME_NONE || timer_timeout <= now || priv->eos) {
      ...
      id = jbtimers->clock_id = gst_clock_new_single_shot_id (jbtimers->clock, sync_time);
  ...
  if (base_rtptime == -1 || clock_rate == -1 || base_time == -1) {
    ...
    if (base_rtptime > ext_rtptime) {
      ...
      if (ext_rtptime > last_rtptime) {
        ...
        if (priv->max_rtcp_rtp_time_diff != -1 &&
            diff >
            gst_util_uint64_scale (priv->max_rtcp_rtp_time_diff,
                clock_rate, 1000)) {
    ...
    default:
      if (GST_QUERY_IS_SERIALIZED (query)) {
        ...
        if (rtp_jitter_buffer_get_mode (priv->jbuf) !=
          ...
          item = alloc_item (query, ITEM_TYPE_QUERY, GST_CLOCK_TIME_NONE, GST_CLOCK_TIME_NONE, -1, 0, -1);
          rtp_jitter_buffer_insert (priv->jbuf, item, &head, NULL);
          if (head)
            JBUF_SIGNAL_EVENT (priv);
