...
rtp_session_class_init (RTPSessionClass * klass)
{
  ...
  rtp_session_signals[SIGNAL_ON_SSRC_PSE] =
      ...
      RTP_TYPE_SOURCE, G_TYPE_UINT, GST_TYPE_BUFFER, G_TYPE_UINT);
  /**
  ...
  rtp_session_signals[SIGNAL_ON_CREATING_SRRR] =
      ...
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (RTPSessionClass, on_creating_srrr),
  ...
  g_object_class_install_property (gobject_class, PROP_SSRC_COLLISION_DETECTION,
      g_param_spec_boolean ("ssrc-collision-detection", "SSRC collision detection",
  ...
  g_object_class_install_property (gobject_class, PROP_STATS_MIN_INTERVAL,
      g_param_spec_uint ("stats-min-interval", "Minimum stats interval",
          ...
          0, G_MAXUINT, DEFAULT_STATS_MIN_INTERVAL,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
rtp_session_init (RTPSession * sess)
{
  ...
  sess->stats_min_interval_ms = DEFAULT_STATS_MIN_INTERVAL;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MAX_MISORDER_TIME:
      ...
      break;
    case PROP_STATS_MIN_INTERVAL:
      sess->stats_min_interval_ms = g_value_get_uint (value);
      break;
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_STATS:
      ...
      break;
    case PROP_STATS_MIN_INTERVAL:
      g_value_set_uint (value, sess->stats_min_interval_ms);
      break;
    case PROP_RTP_PROFILE:
    ...
    GstRTCPPacket * packet, RTPPacketInfo * pinfo)
{
  ...
  if (!gst_rtcp_packet_get_profile_specific_ext (packet, &pse_data, &pse_length))
    return;

  if (!g_signal_has_handler_pending (sess,
        rtp_session_signals[SIGNAL_ON_SSRC_PSE], 0, TRUE))
    return;
  ...
  pse_buffer = gst_buffer_copy_region (packet->rtcp->buffer,
      GST_BUFFER_COPY_MEMORY, pse_data - packet->rtcp->map.data,
      pse_length);
  ...
  g_object_ref (source);
  RTP_SESSION_UNLOCK (sess);
  g_signal_emit (sess, rtp_session_signals[SIGNAL_ON_SSRC_PSE], 0,
      source, type, pse_buffer, rb_count);
  RTP_SESSION_LOCK (sess);
  g_object_unref (source);
    ...
    GstClockTime current_time)
{
  ...
  if (!src->recv_xpli) {
    ...
    if (rtp_session_request_local_key_unit (sess, src, media_ssrc, FALSE,
            current_time)) {
...
is_rtcp_time (RTPSession * sess, GstClockTime current_time, ReportData * data)
{
  ...
  if (!data->is_early) {
     ...
     */
    if (sess->last_rtcp_send_time != GST_CLOCK_TIME_NONE && interval != GST_CLOCK_TIME_NONE) {
      GstClockTime elapsed = current_time - sess->last_rtcp_send_time;

      if (elapsed > interval) {
        data->is_early = FALSE;
        GST_DEBUG_OBJECT (sess, "Ajusting early request: current_time %" GST_TIME_FORMAT " last_rtcp_send_time %" GST_TIME_FORMAT
            " elapsed %" GST_TIME_FORMAT " interval %" GST_TIME_FORMAT " updated is_early = %s",
            GST_TIME_ARGS (current_time), GST_TIME_ARGS (sess->last_rtcp_send_time),
            GST_TIME_ARGS (elapsed), GST_TIME_ARGS (interval), data->is_early ? "TRUE" : "FALSE");
      }
    }
  }
    ...
    guint64 ntpnstime, GstClockTime running_time)
{
  ...
  if (!GST_CLOCK_TIME_IS_VALID (sess->last_stats_notify_time) ||
      (GST_CLOCK_DIFF (sess->last_stats_notify_time, current_time) >=
          sess->stats_min_interval_ms * GST_MSECOND)) {
...
rtp_session_request_fir (RTPSession * sess, guint32 ssrc, gint count)
{
  ...
  GST_DEBUG ("request FIR for %08x", ssrc);
