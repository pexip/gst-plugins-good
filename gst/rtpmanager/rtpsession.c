...
rtp_session_init (RTPSession * sess)
{
  ...
  sess->twcc = rtp_twcc_manager_new ();
}
...
update_packet (GstBuffer ** buffer, guint idx, RTPPacketInfo * pinfo)
{
  ...
  if (pinfo->rtp) {
    ...
    if (idx == 0) {
      ...
      if (pinfo->twcc_seqnum_ext_id > 0 && !pinfo->send) {
        gpointer data;
...
process_twcc_packet (RTPSession * sess, RTPPacketInfo * pinfo)
{
  ...
  rtp_twcc_manager_recv_packet (sess->twcc, pinfo);

  /* we send a feedback at the end of each frame */
  if (pinfo->marker) {
    sess->twcc_send_fb_msg = TRUE;
    RTP_SESSION_UNLOCK (sess);
    if (!rtp_session_send_rtcp (sess, 0)) {
      GST_WARNING ("Could not send TWCC");
    }
    RTP_SESSION_LOCK (sess);
  }
}
...
{
  if (sess->twcc_seqnum_ext_id == 0)
    return;

  /* FIXME: what to do with lists? */
  g_assert (is_list == FALSE);

  rtp_twcc_manager_send_packet (sess->twcc,
      pinfo, sess->twcc_seqnum_ext_id);
}
    ...
    GstClockTime current_time, GstClockTime running_time)
{
  ...
  send_twcc_packet (sess, data, is_list, &pinfo);
...
session_twcc (RTPSession * sess, ReportData * data)
{
  ...
  if (!gst_rtcp_buffer_add_packet (rtcp, GST_RTCP_TYPE_RTPFB, packet)) {
    /* exit because the packet is full, will put next request in a
     * further packet */
    GST_WARNING ("RTCP packet full! Could not fit TWCC stuff");
    return;
  }
  ...
  rtp_twcc_manager_add_fci (sess->twcc, packet);

  sess->twcc_send_fb_msg = FALSE;
}
