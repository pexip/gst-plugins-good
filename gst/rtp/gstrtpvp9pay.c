    ...
    gsize buffer_size)
{
  ...
  if (keyframe) {
    ...
    GST_INFO_OBJECT (self, "parsed width=%d height=%d", self->width, self->height);
  }
    ...
    gboolean start, gboolean mark, GstBuffer * in)
{
  ...
  out = gst_rtp_base_payload_allocate_output_buffer (
      GST_RTP_BASE_PAYLOAD_CAST (self), hdrlen, 0, 0);
  ...
  if (self->is_keyframe && start) {
    p[0] |= 0x02;
    /* scalability structure, hard coded for now to be similar to chromium for quick and dirty interop */
