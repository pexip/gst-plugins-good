...
static void 
gst_multiudpsink_control_msg_receiver_start(GstMultiUDPSink * sink){

}


static gpointer
gst_multiudpsink_control_msg_v4_receiver_thread(gpointer data){
  GError * error = NULL;
  GstMultiUDPSink * sink = GST_MULTIUDPSINK(data);

  while (sink->control_msg_receiver_stop == FALSE){
    if (g_socket_condition_timed_wait (sink->used_socket, G_IO_IN, 100000 /* 100 ms */, NULL, &error)){
      if (g_socket_condition_check(sink->used_socket, G_IO_IN) & G_IO_ERR){
          gst_multiudpsink_control_msg_reader(sink, sink->used_socket);
      }
    }    
  }
}
...
static gpointer
gst_multiudpsink_control_msg_v6_receiver_thread(gpointer data){
  GError * error = NULL;
  GstMultiUDPSink * sink = GST_MULTIUDPSINK(data);

  while (sink->control_msg_receiver_stop == FALSE){
    if (g_socket_condition_timed_wait (sink->used_socket_v6, G_IO_IN, 100000 /* 100 ms */, NULL, &error)){
      if (g_socket_condition_check(sink->used_socket_v6, G_IO_IN) & G_IO_ERR){
          gst_multiudpsink_control_msg_reader(sink, sink->used_socket_v6);
      }
    }
  }
}
...
static void
gst_multiudpsink_control_msg_reader(GstMultiUDPSink * sink, GSocket * socket){
    GError * error = NULL;
    gint ret;
    const guint message_count = 16; //Max number of messages we will read at a time.
    GInputMessage message[message_count];
    GSocketControlMessage ** control_messages[message_count];
    guint num_control_messages[message_count];
    ...
    ret = g_socket_receive_messages(recv->socket.gsocket, message, message_count, MSG_ERRQUEUE, NULL, &error);
    if (ret == -1){
    ...
    for (int i=0; i < ret; i++){
      gst_multiudpsink_control_msg_parser(sink, socket, &message[i]);
...
static void
gst_multiudpsink_control_msg_parser(GstMultiUDPSink * sink, GSocket * socket, GInputMessage * message){
