...
static void gst_multiudpsink_timestamping_reader (GstMultiUDPSink * sink,
    GSocket * socket);
static void gst_multiudpsink_timestamping_parser (GstMultiUDPSink * sink,
    GSocket * socket, GInputMessage * message);

#define gst_multiudpsink_timestamping_parent_class parent_class
G_DEFINE_TYPE (GstMultiUDPSinkTimestamping, gst_multiudpsink_timestamping,
    G_TYPE_OBJECT);
...
static void
gst_multiudpsink_timestamping_class_init (GstMultiUDPSinkTimestampingClass *
    klass)
{
...
{
  GError *error = NULL;
  struct _gst_multiudpsink_timestamping_priv_data *priv_data =
      (struct _gst_multiudpsink_timestamping_priv_data *) data;
  g_assert (priv_data);

  while (g_cancellable_is_cancelled (priv_data->sink->timestamping->shutdown) ==
      FALSE) {
    if (g_socket_condition_wait (priv_data->socket, G_IO_ERR,
            priv_data->sink->timestamping->shutdown, &error)) {
      if (g_socket_condition_check (priv_data->socket, G_IO_ERR) & G_IO_ERR) {
        gst_multiudpsink_timestamping_reader (priv_data->sink,
            priv_data->socket);
      }
    }
  }
...
gst_multiudpsink_timestamping_reader (GstMultiUDPSink * sink, GSocket * socket)
{
  ...
  ret =
      g_socket_receive_messages (socket, message, CONTROL_MSG_MAX_MESSAGES,
      MSG_ERRQUEUE, NULL, &error);
  if (ret == -1) {
    if (error->code == G_IO_ERROR_WOULD_BLOCK) {
      g_warning
          ("receiver: g_socket_receive_messages (IO_ERR) unexpectently returned G_IO_ERROR_WOULD_BLOCK: %s\n",
          error->message);
    } else {
      GST_ERROR ("receiver: g_socket_receive_messages (IO_ERR): %s\n",
          error->message);
    }
    return;
  }
...
static void
gst_multiudpsink_timestamping_parser (GstMultiUDPSink * sink, GSocket * socket,
    GInputMessage * message)
{
  ...
  if (*message->num_control_messages != 2) {
    GST_ERROR_OBJECT (sink, "Expected number of control messages returned to be 2, but got %d!\n",
        *message->num_control_messages);
    return;
  }

  if ((g_unix_timestamping_unify_control_message_set (*message->
              control_messages, &unified)) == -1) {
    GST_ERROR_OBJECT (sink, "Failed to unify control message set!\n");
    return;
  }

  GST_DEBUG_OBJECT (sink, "Type:%u TypeName:%s Source:%u SourceName:%s PacketID: %u TS: %ld.%09ld\n",
      unified.timestamping_type,
      g_unix_timestamping_get_timestamping_type_name (unified.
          timestamping_type), unified.timestamping_source,
      g_unix_timestamping_get_timestamping_source_name (unified.
          timestamping_source), unified.packet_id, unified.timestamping_sec,
      unified.timestamping_nsec);

  ts = ((GstClockTime) unified.timestamping_sec * GST_SECOND) +
      unified.timestamping_nsec;
  s = gst_structure_new ("GstMultiUDPTimestamping",
      ...
      NULL);
  gst_element_send_event (GST_ELEMENT_CAST (sink),
...
gst_multiudpsink_timestamping_new (GstMultiUDPSink * sink)
{
  ...
  priv_data =
      g_malloc0 (sizeof (struct _gst_multiudpsink_timestamping_priv_data));
  g_assert (priv_data);
  priv_data->sink = sink;
  priv_data->socket =
      (sink->used_socket) ? sink->used_socket : sink->used_socket_v6;
  snprintf (thread_name, GST_MULTIUDPSINK_TIMESTAMPING_NAME_MAXLEN, "%s_sd_%d",
      GST_MULTIUDPSINK_TIMESTAMPING_NAME_PREFIX,
      g_socket_get_fd (priv_data->socket));

  handle = g_object_new (GST_TYPE_MULTIUDPSINK_TIMESTAMPING, NULL);
  g_assert (handle);
  handle->thread =
      g_thread_new (thread_name, gst_multiudpsink_timestamping_receiver_thread,
      priv_data);
